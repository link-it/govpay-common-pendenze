...
6. **`SOLUZIONE_UNICA` con più di una pendenza — risolto (2026-09-22).** Trovato
   in un commento (non nel body, non nello YAML as-is) dell'issue
   ...
   cambia solo il layout di stampa degli avvisi"*. **Confermato dal lead**: lo
   YAML va aggiornato di conseguenza (oggi impone ancora `maxItems: 1`) e la
   casistica va gestita. Implementato in `TipologiaOpzionePagamento`:
   `SOLUZIONE_UNICA` ora senza limite massimo di pendenze (come
   `PIANO_RATEALE`); `SOLUZIONE_UNICA_ENTRO`/`OLTRE` restano a esattamente 1 —
   **da confermare se il requisito vale anche per queste due**, la richiesta
   discussa riguardava solo `SOLUZIONE_UNICA`.
